...
class BillingManager(
                    ...
                    for (purchase in purchases) {
                        when (purchase.purchaseState) {
                            ...
                            Purchase.PurchaseState.PENDING -> {
                                // Here you can confirm to the user that they've started the pending purchase, and to complete it, they should follow instructions that
                                // are given to them. You can also choose to remind the user in the future to complete the purchase if you detect that it is still
                                // pending.
                            }
                        }
                    }
    ...
    fun querySubscription(onSuccess: (List<SkuDetails>) -> Unit, onError: (code: Int, message: String) -> Unit) {
        startServiceConnection {
            querySubscriptionSkuDetails(listOf(
                Sku.ADFREE,
                Sku.ADFREEUNLIMITEDLIST
            ), onSuccess, onError)
        }
    }
    ...
    object Sku {
        const val ADFREE = "gsledger_subscribe" // Made a mistake with "business_monthly", accidentally make it yearly, so we can't use that SKU anymore
        const val ADFREEUNLIMITEDLIST = "adfree_unlimited_entry"
